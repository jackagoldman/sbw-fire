...
Hypothesis 1: There is a difference in median severity between defoliated and non defoliated

```{r, echo = FALSE}
targets::tar_read(sev_ttest) |> dplyr::filter(Test == "Median Severity")

```

Hypothesis 2: These is a difference in severity extremes between defoliated and non defoliated
```{r, echo = FALSE}
targets::tar_read(sev_ttest) |> dplyr::filter(Test == "Extreme Severity")

```

Hypothesis 3: These is a difference in variability in burn severity between defoliated and non defoliated
```{r, echo = FALSE}
targets::tar_read(sev_ttest) |> dplyr::filter(Test == "Variability in Severity")

```


```{r}
targets::tar_read(boxplot)
```
